...
#undef minor
	...
	public:									// changed to make draw_lines() invokable
	...
	struct Ellipse : Shape {
		...
		Point center() const { return{ point(0).x + w, point(0).y + h }; }			// CS 2nd/01/2022 Incorrect Default Function
		...
		int major() const { return w; }				// CS major is just the x-axis, not the axis corresponding to the larger diameter
		void set_minor(int hh) { h = hh; }
		int minor() const { return h; }				// CS minor is just the y-axis, not the axis corresponding to the smaller diameter
	private:
		int w;
		int h;
	};
